...
// ---- 全チェック完了時の背景画像URL（ローカル優先。未設定なら /hawaii.png を参照） ----
...
// 未チェック→先頭、チェックは _checkedAt の新しい順で下に積む
...
export default function App() {
  ...
  const toggleCheckedById = (id: string) => {
    ...
    if (madeChecked && !state.edit) { requestAnimationFrame(() => { const w = checkedWrapRef.current; if (!w) return; w.scrollTo({ top: w.scrollHeight, behavior: 'smooth' }); setTimeout(() => { try { w.scrollTo({ top: w.scrollHeight, behavior: 'smooth' }); } catch {} }, 60); }); }
  };
  ...
  // Reset（二段階）
  const [resetArmed, setResetArmed] = useState(false); const resetTimerRef = useRef<number | null>(null); const ARM_TIMEOUT_MS = 5000;
  ...
  function updatePlaceholder(list: HTMLDivElement, clientY: number, draggingIds: string[], totalH: number) {
    ...
    const ref = children[targetIndex] as HTMLElement | undefined; const phH = Math.max(28, totalH - ROW_SPACING);
  ...
  function finalizePlaceholder(list: HTMLDivElement, draggingIds: string[]) {
    ...
    const all = Array.from(list.children); const phPos = all.indexOf(ph); const insertAt = all.slice(0, phPos).filter((el) => { const he = el as HTMLElement; const id = he.dataset?.id || ""; const isPh = he.dataset?.placeholder === "1"; const isCollapsed = he.dataset?.dragCollapsed === "1" || (he as HTMLElement).style.display === "none"; return !isPh && !isCollapsed && !draggingIds.includes(id); }).length; return { insertAt };
  }
  ...
  return (
    <div
      style={{ display: "grid", gridTemplateRows: state.edit ? "auto 1fr" : `auto ${CHECKED_VH}vh 1fr`, height: "100vh" }}
      onPointerDown={() => { primeHaptics(); }} onMouseDown={() => { primeHaptics(); }} onTouchStart={() => { primeHaptics(); }}
    >
      {/* Header: Progress + Toolbar（右寄せ/Reset→Undo→Redo→編集 の順） */}
      ...
      {/* Checked Area（通常時のみ表示・固定高さ） */}
      {!state.edit && (
        <div ref={checkedWrapRef} style={{ overflow: "auto", padding: 10, borderBottom: "1px solid #eef2f7", background: "#f8fafc" }}>
          <div ref={checkedListRef} style={{ display: "flex", flexDirection: "column", gap: 0 }}>
            {checkedItemsAsc.map((item, i) => (
              <Row key={item.id} item={item} index={i} edit={state.edit} onInput={() => {}} onEnter={() => {}} onToggleChecked={() => toggleCheckedById(item.id)} onToggleSelected={() => {}} onDelete={() => { if (state.edit) deleteItem(item.id); }} onPointerDownHandle={() => {}} allCheckedBlue={false} onEndTyping={() => {}} />
            ))}
          </div>
        </div>
      )}

      {/* Unchecked Area（片手操作しやすい広め領域） */}
      <div
        ...
        style={{
          ...
          backgroundImage: allCheckedBlue ? `url(${getAllDoneBgUrl()})` : undefined,
...
function Row(props: {
  ...
  onPointerDownHandle: (e: React.PointerEvent<HTMLElement>) => void; onPointerDownFromSelect?: (e: React.PointerEvent<HTMLElement>) => void; onEndTyping: () => void; allCheckedBlue: boolean;
}) {
  const { item, index, edit, allCheckedBlue } = props; const taRef = useRef<HTMLTextAreaElement | null>(null);
  useEffect(() => { const ta = taRef.current; if (!ta) return; const MIN = 24; ta.style.padding = "0"; ta.style.height = "0px"; const h = Math.max(MIN, Math.min(ta.scrollHeight, window.innerHeight * 0.35)); ta.style.height = h + "px"; }, [item.text, edit]);
  const bgColor = allCheckedBlue ? "#93c5fd" : (item.checked ? "#cbd5e1" : "#fff");
  const rowStyle: React.CSSProperties = { display: "flex", alignItems: "center", flexWrap: "nowrap", marginTop: index === 0 ? 0 : ROW_SPACING, border: "1px solid #e5e7eb", borderRadius: 10, padding: "6px 16px", gap: 6, width: "100%", background: bgColor, boxSizing: "border-box", touchAction: "none" };
  if (edit && item.checked) { return (<div className="row" data-id={item.id} data-index={index} data-checked="1" style={rowStyle}><textarea ref={taRef} value={item.text} readOnly spellCheck={false} onBlur={() => props.onEndTyping()} style={{ flex: 1, minWidth: 0, border: "none", outline: "none", resize: "none", background: "transparent", color: "#777", textDecoration: "line-through", padding: "0", fontSize: 16, lineHeight: "24px", minHeight: "24px", alignSelf: "center" }} /></div>); }
  return (
...
function runTests() {
  ...
  // 3) sortUncheckedFirst order
  tests.push({ name: "sortUncheckedFirst order", fn: () => {
    const a: Item = { id: "a", text: "a", checked: true };
    const b: Item = { id: "b", text: "b", checked: false };
  ...
  // 8) sortUncheckedFirst checked by _checkedAt desc
  ...
  // 9) reorderByInsert clamps insertAt
  ...
  for (const t of tests) {
    try { t.fn(); passed++; } catch (e: any) { failed++; failures.push(`${t.name}: ${e?.message || e}`); }
  }
