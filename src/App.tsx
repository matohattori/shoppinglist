...
const vibrateSafe = (p: number | number[]) => {
  try {
    if ("vibrate" in navigator) {
      ...
      return !!ok || ok === undefined; // 一部端末は undefined を返す
    }
  } catch {}
  return false;
};
...
// unchecked first (used for normal mode & edit view order)
function sortUncheckedFirst(items: Item[]) {
  const a: Item[] = [],
    b: Item[] = [];
  for (const it of items) (it.checked ? b : a).push(it);
  b.sort((x, y) => (y._checkedAt ?? 0) - (x._checkedAt ?? 0)); // checked: newest first (上から積む)
  return a.concat(b);
}
...
// ========================= App (with grouped typing history) =========================
export default function App() {
  ...
  // order view: always unchecked first
